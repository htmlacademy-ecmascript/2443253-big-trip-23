...
import {OFFERS,EVENT_TYPES} from '../const.js';
import {createDestinationImage} from './form-create-view.js';


const CHECKED_INDEX = [5];

function createEventTypeItem(typeItem,state = ''){
...
function createEventOfferItem({name,price},checked){
  return `
    ...
    offers.includes(element) ? createEventOfferItem(element,true) : createEventOfferItem(element,false)).join('')}
      ...
      <section class="event__section  event__section--destination">
        ...
        <div class="event__photos-tape">
          ${createDestinationImage(destination.foto)}
        </div>
      </div>
...
export {createEventTypeItem,createEventOfferItem,CHECKED_INDEX};
