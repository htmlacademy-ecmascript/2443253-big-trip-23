...
export default class SortListView extends AbstractView{
  ...
  #createSortItemTemplate(element, state = '') {
    const {type: sortingType, count} = element;
    ...
    <label class="trip-sort__btn" for="sort-${sortingType}">${capitalize(sortingType)} ${count}</label>
  </div>`;
  }

  get template() {
    return `<form class="trip-events__trip-sort  trip-sort" action="#" method="get">
    ${this.#sorters.map((element,index) => this.#createSortItemTemplate(element,this.#STATES_SORT[index])).join('')
}
  </form>`;
